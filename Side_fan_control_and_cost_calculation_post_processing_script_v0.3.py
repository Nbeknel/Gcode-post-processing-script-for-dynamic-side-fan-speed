...
for notes in ["printer_notes", "notes", "filament_notes"]:
    for feature_type, feature in feature_types.items():
        match = re.search(fr"""pps_{feature[slicer].replace(" ", "_").lower()}_cost_multiplier=(\d+\.?\d*|\.\d+)(\n|$)""", slicer_variables[notes][value])
        if match:
            feature_types[feature_type]["Cost_multiplier"] = float(match.group(1))

    for variable_name, variable in variables.items():
        match = re.search(fr"""pps_{variable_name}=(\d+\.?\d*|\.\d+)(\n|$)""", slicer_variables[notes][value])
        if match:
            variables[variable_name][value] = float(match.group(1))
    
    match = re.search(fr"pps_fan_max_positive_step=(\d+\.?\d*|\.\d+)(\n|$)", slicer_variables[notes][value])
    if match:
        fan_max_positive_step = float(match.group(1))
    
    match = re.search(fr"pps_fan_max_negative_step=(\d+\.?\d*|\.\d+)(\n|$)", slicer_variables[notes][value])
    if match:
...
def get_time(speed, accel, x_delta, y_delta, z_delta, e_delta):
    speed = max(1, speed / 60)
    accel = max(1, accel)
    ...
    if distance > distance_to_reach_target_velocity:
        return 0.5 * ((2 * distance - distance_to_reach_target_velocity) / speed + 2 * speed / accel)
    return math.sqrt(distance / accel) + 0.5 * distance / speed
    #    return (distance - distance_to_reach_target_velocity) / speed + 2 * speed / accel
...
for line in lines:
    ...
    if x_delta ** 2 + y_delta ** 2 + z_delta ** 2 + e_delta ** 2 > 0:
        layer.add_time(get_time(speed, accel, x_delta, y_delta, z_delta, e_delta))
